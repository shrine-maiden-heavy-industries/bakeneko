...
def test(session: Session) -> None:
	...
	def _setup_test_env() -> None:
		...
		if (remote_cfg := cfg.get('ssh')) is not None:
			if (host := remote_cfg.get('host')) is not None:
				env['BAKENEKO_REMOTE_TEST_HOST'] = host

			if (user := remote_cfg.get('user')) is not None:
				env['BAKENEKO_REMOTE_TEST_USER'] = user

			if (key := remote_cfg.get('key')) is not None:
				env['BAKENEKO_REMOTE_TEST_KEY'] = key

		if (debug_cfg := cfg.get('serial')) is not None:
			if (port := debug_cfg.get('port')) is not None:
				env['BAKENEKO_SERIAL_TEST_PORT'] = port
...
def build_docset(session: Session) -> None:
	...
	# Get the Bakeneko version
	bakeneko_version: str = session.run(
		'python', '-c', 'import bakeneko;print(bakeneko.__version__)',
		silent = True
	)

	with session.chdir(BUILD_DIR):
		# If the docset is already built, shred it because `doc2dash` won't overwrite it
		if (BUILD_DIR / 'Bakeneko.docset').exists():
			rmtree(BUILD_DIR / 'Bakeneko.docset')

		# Build the docset
		session.run(
			'doc2dash', '-n', 'Bakeneko', '-j', '--full-text-search', 'on', str(DOCS_DIR)
		)

		# Compress it
		make_archive(f'bakeneko-{bakeneko_version.strip()}-docset', 'zip', BUILD_DIR, 'Bakeneko.docset')
...
def dist_docs(session: Session) -> None:
	...
	with session.chdir(BUILD_DIR):
		make_archive(f'bakeneko-{bakeneko_version.strip()}-docs', 'zip', BUILD_DIR, 'docs')
...
def typecheck_mypy(session: Session) -> None:
	...
	session.run(
		...
		'-p', 'bakeneko', '--html-report', str(OUTPUT_DIR.resolve())
	)
